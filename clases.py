...
class Detector:
    ...
    def __init__(self, adn_list):
        try:
            ...
            self.analizar_mutaciones()  # Llama automáticamente a los métodos de análisis
            self.resumen()
        except IndexError:
    ...
    def detectar_mutantes_horizontal(self):
        for i in range(len(self.adn_list)):
            ...
            for j in range(len(cadena) - 3):  # Verificar hasta 4 caracteres antes del final
                if cadena[j] == cadena[j+1] == cadena[j+2] == cadena[j+3]:
                    self.mutacion_horizontal += 1
                    self.mutacion = True
                    print(f"Mutación horizontal encontrada en la fila {i} posición {j}-{j+3}")
                    break  # Salir al encontrar la primera mutación en la fila

    def detectar_mutantes_vertical(self):
        for j in range(len(self.adn_list[0])):
            for i in range(len(self.adn_list) - 3):  # Verificar hasta 4 filas antes del final
                if (self.adn_list[i][j] == self.adn_list[i+1][j] == self.adn_list[i+2][j] == self.adn_list[i+3][j]):
                    self.mutacion_vertical += 1
                    self.mutacion = True
                    print(f"Mutación vertical encontrada en la columna {j} posición {i}-{i+3}")
                    break

    def detectar_mutantes_diagonal(self):
        for i in range(len(self.adn_list) - 3):
            for j in range(len(self.adn_list[0]) - 3):  # Verificar hasta 4 columnas antes del final
                if (self.adn_list[i][j] == self.adn_list[i+1][j+1] == self.adn_list[i+2][j+2] == self.adn_list[i+3][j+3]):
                    self.mutacion_diagonal += 1
                    self.mutacion = True
                    print(f"Mutación diagonal encontrada desde ({i},{j}) hasta ({i+3},{j+3})")
                    break

    def detectar_mutantes_diagonal_inverso(self):
        for i in range(len(self.adn_list) - 3):
            for j in range(3, len(self.adn_list[0])):  # Comenzar desde la columna 3 hasta el final
                if (self.adn_list[i][j] == self.adn_list[i+1][j-1] == self.adn_list[i+2][j-2] == self.adn_list[i+3][j-3]):
                    self.mutacion_diagonal += 1
                    self.mutacion = True
                    print(f"Mutación diagonal inversa encontrada desde ({i},{j}) hasta ({i+3},{j-3})")
                    break
...
class Radiacion(Mutador):
    ...
    def crear_mutantes_horizontal(self):
        for posicion_vertical in range(len(self.adn_list)):
    ...
    def crear_mutantes_vertical(self):
        for posicion_horizontal in range(len(self.adn_list[0])):
...
class Virus(Mutador):
    ...
    def crear_mutantes_diagonal(self):
        for self.posicion_vertical in range(len(self.adn_list) - 3):
            for self.posicion_horizontal in range(len(self.adn_list[0]) - 3):  # Verificar hasta 4 columnas antes del final
    ...
    def crear_mutantes_diagonal_inverso(self):
        for self.posicion_vertical in range(len(self.adn_list) - 3):
            for self.posicion_horizontal in range(3, len(self.adn_list[0])):  # Comenzar desde la columna 3 hasta el final
                if (self.adn_list[self.posicion_vertical][self.posicion_horizontal] == self.adn_list[self.posicion_vertical+1][self.posicion_horizontal-1] == self.adn_list[self.posicion_vertical+2][self.posicion_horizontal-2]):
                    self.adn_list[self.posicion_vertical+3][self.posicion_horizontal-3] = self.adn_list[self.posicion_vertical][self.posicion_horizontal]
                    self.viruses= True
                    break
...
class Sanador:
    ...
    def analizar_mutaciones(self):
        while True:
            self.mutacion = False  # Reinicia el estado de mutación
            self.detectar_mutantes_horizontal()
            self.detectar_mutantes_vertical()
            self.detectar_mutantes_diagonal()
            self.detectar_mutantes_diagonal_inverso()
        ...
        # Imprime el ADN final si no se detectaron mutaciones
        print(f"""ADN:
      {self.adn_list[0]}
      {self.adn_list[1]}
      {self.adn_list[2]}
